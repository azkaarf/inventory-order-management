-- Database terpisah khusus buat integration test (TEST-02), supaya
-- test nggak pernah menyentuh data development di database 'inventory'.
-- Isinya sengaja SAMA PERSIS dengan schema-and-seed.sql (tabel + seed),
-- supaya ada baseline data (mis. minimal 1 user) buat test yang butuh.
CREATE DATABASE IF NOT EXISTS inventory_test;

-- inventory_user dibuat otomatis oleh image MySQL, tapi HANYA diberi akses
-- ke database MYSQL_DATABASE (inventory) secara default - jadi perlu
-- di-grant manual ke database test ini juga. Kalau kamu ganti DB_USER
-- di .env, sesuaikan juga nama user di GRANT ini.
GRANT ALL PRIVILEGES ON inventory_test.* TO 'inventory_user'@'%';
FLUSH PRIVILEGES;
